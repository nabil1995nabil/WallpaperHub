...
if(heroEditProfileBtn){
    heroEditProfileBtn.onclick = () => {
        ...
        if(bioInput){
            bioInput.value = String(
                localStorage.getItem("profileBio") || ""
            ).slice(0, 40);
            bioInput.maxLength = 40;
        }
    };
}
...
const bio =
Array.from(String(editBio?.value || "").trim())
    .slice(0, 40)
    .join("");
...
if(infoUserName && name)
    infoUserName.textContent = name;

if(heroDisplayName && name)
    heroDisplayName.textContent = name;

if(heroBio)
    heroBio.textContent =
        bio || "مصمم خلفيات ومحب للتصميم ✨";
